...
> `pwd`/previous.html;
cat `pwd`/header.prev | sed -e "s#Date#Для периода с <b>$(date -d "`date +%Y-%m-%d` - `expr $(date +%u) - 1 - 7` days" +%d-%m-%Y)</b> по <b>$(date -d "`date +%Y-%m-%d` - `expr $(date +%u) - 1 - 7` days + 7 days" +%d-%m-%Y)</b>#g" >> `pwd`/previous.html;
...
touch `pwd`/next.html;
> `pwd`/next.html;
cat `pwd`/header.next | sed -e "s#Date#Для периода с <b>$(date -d "`date +%Y-%m-%d` - `expr $(date +%u) - 1 + 7` days" +%d-%m-%Y)</b> по <b>$(date -d "`date +%Y-%m-%d` - `expr $(date +%u) - 1 + 7` days + 7 days" +%d-%m-%Y)</b>#g" >> `pwd`/next.html;
